...
<script lang="ts">
    ...
    export default Vue.extend({
      ...
      async asyncData({ params, query, $api, route, store }) {
        ...
        const isTableView = (store.state as RootState).core.tableCategoryView && !(query.gene || query.trait || query.shop || (qs.parse(route.fullPath.replace(route.path, '').replace('?', '')).morphs && qs.parse(route.fullPath.replace(route.path, '').replace('?', '')).morphs.length))
